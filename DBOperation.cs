...
namespace DataBase
{
    ...
    public class DBOperator
    {
        ...
        private List<SQLiteParameter> listParam;
        public DBOperator()
        {
            connectionString = ConfigurationManager.ConnectionStrings["SQLConnString"].ConnectionString;
            listParam = new List<SQLiteParameter>();
        }
        public DBOperator(string _connectionstring)
        {
            connectionString = _connectionstring;
            listParam = new List<SQLiteParameter>();
        }

        public void NewSQL(string sqltxt)
        {
            sql = sqltxt;
            listParam.Clear();
        }
        ...
        {
            SQLiteParameter p = new SQLiteParameter(name, DbType.Int32);
            p.Value = value;
            listParam.Add(p);
        }
        ...
        {
            Int32 t = Convert.ToInt32(value);
            SQLiteParameter p = new SQLiteParameter(name, DbType.Int32);
            p.Value = t;
            listParam.Add(p);
        }
        public void SetTime(string name, System.DateTime value)
        {
            string t = value.ToString("s");
            SQLiteParameter p = new SQLiteParameter(name, DbType.String);
            p.Value = t;
            listParam.Add(p);
        }
        ...
        public int ExecuteNonQuery()
        {
            if (listParam.Count == 0)
                return SqliteHelper.ExecuteNonQuery(connectionString, CommandType.Text, sql, null);
            else
            {
                param = listParam.ToArray();
                return SqliteHelper.ExecuteNonQuery(connectionString, CommandType.Text, sql, param);
            }
        }
        ...
        {
            if (listParam.Count == 0)
                return SqliteHelper.ExecuteReader(connectionString, CommandType.Text, sql, null);
            else
            {
                param = listParam.ToArray();
                return SqliteHelper.ExecuteReader(connectionString, CommandType.Text, sql, param);
            }
        }
        ...
        {
            if (listParam.Count == 0)
                return SqliteHelper.ExecuteDataSet(connectionString, CommandType.Text, sql, null);
            else
            {
                param = listParam.ToArray();
                return SqliteHelper.ExecuteDataSet(connectionString, CommandType.Text, sql, param);
            }
        }
        ...
        {
            if (listParam.Count == 0)
                return SqliteHelper.ExecuteScalar(connectionString, CommandType.Text, sql, null);
            else
            {
                param = listParam.ToArray();
                return SqliteHelper.ExecuteScalar(connectionString, CommandType.Text, sql, param);
            }
        }
